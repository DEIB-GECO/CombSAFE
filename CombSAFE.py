...
def get_semantic_annotations(geo_id_list):
    ...
    print("Semantic Annotations successfully generated.")
...
def generate_semantic_df(geo_id_list):
...
def plot_distance_matrix(pool):
    ...
    fig.savefig("distance_matrix_OnASSiS.jpeg", dpi=100, bbox_inches = "tight", format="jpeg") 
...
def plot_semantic_driven_dendrogram():
    ...
    fig.savefig("semantic_driven_dedrogram.jpeg", dpi=100, bbox_inches = "tight", format="jpeg") 
...
def identify_chromatin_states(number_of_states, n_core):
    ...
    binarizing = " ".join(['java', '-mx32600M', '-jar', chromeHMM_path + 'ChromHMM.jar', 'BinarizeBed','-peaks', '-center', './ChromHMM/CHROMSIZES/hg38.txt', destination_path, txt_file, binarized_files])
    subprocess.call(binarizing, shell=True)
    
    learning = " ".join(['java', '-mx32600M', '-jar', chromeHMM_path + 'ChromHMM.jar', 'LearnModel', '-p ' + str(n_core),  binarized_files, n_states_model, str(number_of_states), 'hg38'])
...
def data_driven_dendrogram(new_df):
    ...
    fig.savefig(graphs_path + "data_driven_dendrogram.jpeg", dpi=100, bbox_inches='tight', format="jpeg")
...
def add_custom_tracks(custom_file_name, custom_file_path, index = 0):
...
def remove_empty_annotations(reducted_df, threshold_percentage):
    temp_df = reducted_df.copy()
    inactive_chromatin = np.argmax(np.bincount(reducted_df.values.flat))
    indices_trashold = list((temp_df[(temp_df==inactive_chromatin)].count(axis=0)/temp_df.shape[0]*100).where(lambda x : x>=threshold_percentage).dropna().index)
    temp_df.drop(indices_trashold, axis=1, inplace=True)
    return(temp_df)
